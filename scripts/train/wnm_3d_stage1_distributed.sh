#!/usr/bin/env bash
# Stage I: full fine-tuning on multiple nodes.
#
# Run this script on every node. Example for two 8-GPU nodes:
#   MASTER_ADDR=<node-0-ip> NODE_RANK=0 bash scripts/train/wnm_3d_stage1_distributed.sh
#   MASTER_ADDR=<node-0-ip> NODE_RANK=1 bash scripts/train/wnm_3d_stage1_distributed.sh
#
...
exec bash "$SCRIPT_DIR/wnm_3d_stage1.sh" "$@"
